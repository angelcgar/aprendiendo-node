...
describe("fileSistemDatasource Test", () => {
  ...
  test("should save a log in all logs-all.log and medium", () => {
    const logDatasource = new FileSystemDatasource();
    const log = new LogEntity({
      message: "test",
    ...
    logDatasource.saveLog(log);
    const allLogs = fs.readFileSync(`${logPath}/logs-all.log`, "utf-8");
    const mediuLogs = fs.readFileSync(`${logPath}/logs-medium.log`, "utf-8");

    expect(allLogs).toContain(JSON.stringify(log));
    expect(mediuLogs).toContain(JSON.stringify(log));
  });
